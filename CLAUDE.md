...
**Status:** Plans 1 ("Foundation & Auth"), 2 ("Pet Records Core"), 3 ("App shell and home screen", `docs/superpowers/plans/2026-09-14-app-shell-and-home-screen.md`), and 4 ("Pet profile depth", `docs/superpowers/plans/2026-09-14-pet-profile-depth.md`) are all **complete, device-verified, and merged into `master`**. Plan 5 ("Reminders and notifications", `docs/superpowers/plans/2026-09-15-reminders-and-notifications.md`) is **merged into `master` but NOT YET device-verified** — see the "⚠️ Plan 5 device verification outstanding" note right below and `NEXTSTEPS.md`'s top section before touching anything in `src/reminders/`. Plan numbering follows the execution pack's Phase→Plan mapping (Phase 1 = Plan 3, Phase 2 = Plan 4, Phase 3 = Plan 5, Phase 4 = Plan 6 "Calendar", etc.) — do not confuse this with the original design spec's own draft numbering, which called an earlier, since-superseded reminders concept "Plan 3." See `NEXTSTEPS.md` for exact resume state and known gaps.

**⚠️ Plan 5 device verification outstanding.** Two device checks (Task 7's app-rebuild/launch confirmation, and Task 14's full 7-item on-phone checklist — permission deny/grant, a real notification actually arriving, a skipped dose staying visible in history, Done clearing a reminder, snooze hiding a reminder, live cross-listener sync) could not run in the session that built this plan — the phone's ADB Interface was stuck in Windows' "Unknown" USB state throughout (the same intermittent issue documented below under "Local device build environment"), and no amount of software-side troubleshooting (adb server restart, a PnP device reset attempt) fixed it; it needs a physical cable unplug/replug plus unlocking the phone. Everything that *could* be verified without a device passed (`tsc` clean, 115 automated tests including a fresh independent re-verification that the reminder calculation module has zero Firebase/React dependency, one real integration bug found and fixed by the final whole-branch review), but **nothing has confirmed this feature actually works on a real phone** — treat "merged" and "working" as separate claims until someone runs that checklist. Do not start Plan 6 before this is resolved; see `NEXTSTEPS.md`'s top section for the exact checklist to run.
...
`src/theme/theme.ts` (colors, spacing, radii, typography, a shared `shadow` preset) plus `src/components/ui/` (`Button`, `Card`, `ScreenContainer`, `TextField`, `Chip`, `Typography` — `Title`/`Subtitle`/`BodyText`/`MutedText`/`ErrorText`, `AvatarPicker`, `PetSelector`) are the shared design system every screen is built from as of the 2026-09-13 redesign. Palette: teal/health-blue primary (`#0891B2`), warm orange accent (`#F97316`) for primary "add" actions, calm off-white background — chosen for a pet **health** app specifically (not generic "cute pets"), via the `ui-ux-pro-max` skill's design-system search. New screens should compose from `src/components/ui/index.ts`'s exports rather than styling raw `View`/`Text`/`Button`/`TextInput` inline — that inconsistency (every screen hand-rolling its own `{ padding: 24, gap: 12 }`) is exactly what the redesign replaced.
...
- **OUTSTANDING (2026-09-15):** Plan 5's on-device verification never ran
  — see the "⚠️ Plan 5 device verification outstanding" note near the top
  of this file and `NEXTSTEPS.md`'s top section for the exact checklist.
  Do not treat any part of the reminders feature as confirmed-working
  until that checklist runs on a real phone.
